...
//	NSString *type    = [[comp objectAtIndex:0] substringFromIndex:0];
...
- (void)checkRegions {
	...
	for(int i=0; i < _regions.count; i++) {
		...
		if([self isCurrentLocationInsideRegion:rg]) {
		//	NSLog(@"nowInside");
		//	NSLog(@" arrayCount=%d", _lastRegionState.count);
		//	NSLog(@" lastState=%d, nowState=%d", [lastState intValue], CLRegionStateInside);
			if([lastState intValue] != CLRegionStateInside) {	// Insideではない状態からInsideに変化したら
		...
		//	NSLog(@"nowOutside");
		//	NSLog(@" arrayCount=%d", _lastRegionState.count);
		//	NSLog(@" lastState=%d, nowState=%d", [lastState intValue], CLRegionStateOutside);
			if([lastState intValue] != CLRegionStateOutside) {	// Outsideではない状態からOutsideに変化したら
				if([lastState intValue] != CLRegionStateUnknown) {
					[self requestStateForRegion:rg];
